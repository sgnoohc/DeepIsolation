...
def nEvents_train():
  files = glob.glob("/hadoop/cms/store/user/smay/DeepIsolation/features/features_train_*.npz")
...
def generate_test():
  while 1:
    files = glob.glob("/hadoop/cms/store/user/smay/DeepIsolation/features/features_test_*.npz")
    for file in files:
      npzfile = numpy.load(file)
